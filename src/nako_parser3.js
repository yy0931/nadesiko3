...
class NakoParser extends NakoParserBase {
  ...
  ySentenceList () {
    ...
    if (blocks.length === 0) {
      throw new NakoSyntaxError('構文解析に失敗:' + this.nodeToStr(this.peek(), { depth: 1 }),
        line, this.filename)
    }

    return {type: 'block', block: blocks, line, ...offset }
  }
  ...
  ySentence () {
    ...
    if (this.accept(['抜ける'])) {return {type: 'break', line: this.y[0].line, josi: '', ...offset}}
    if (this.accept(['続ける'])) {return {type: 'continue', line: this.y[0].line, josi: '', ...offset}}
  ...
  yDef(type) {
    ...
    if (funcName.type !== 'func')
      {throw new NakoSyntaxError(
        this.nodeToStr(funcName, { depth: 0, typeName: '関数' }) +
    ...
    try {
      ...
      throw new NakoSyntaxError(this.nodeToStr(funcName, { depth: 0, typeName: '関数' }) +
  ...
  yRepeatTime () {
    ...
    let multiline = false
    let block = null
    if (num === null) {num = {type: 'word', value: 'それ', josi: '', line: kai.line, ...offset}}
    if (this.check('ここから')) {
      this.get()
      multiline = true
        ...
        switch (word.type) {
          case 'func': // 関数の代入的呼び出し
            switch (word.meta.josi.length) {
              ...
              case 1:
                return {type: 'func', name: word.name, args: [value], setter: true, line: dainyu.line, josi: '', ...offset}
              default:
                throw new NakoSyntaxError(`引数が2つ以上ある関数『${word.name}』を代入的呼び出しすることはできません。`, dainyu.line, this.filename)
            }
          case 'ref_array': // 配列への代入
            return {type: 'let_array', name: word.name, index: word.index, value: value, line: dainyu.line, josi: '', ...offset}
          default:
            return {type: 'let', name: word, value: value, line: dainyu.line, josi: '', ...offset}
        }
      }
      if (this.check('定める')) {
        const dainyu = this.get()
  ...
  yCallFunc () {
    ...
    const funcNode = {type: 'func', name: t.value, args: args, josi: t.josi, line: t.line, ...offset}
    // 言い切りならそこで一度切る
    if (t.josi === '')
      {return funcNode}
    ...
    if (this.check2(['func', 'eq'])) {
      ...
      try {
        if (this.accept(['func', 'eq', this.yCalc])) {
          ...
          {throw new NakoSyntaxError(
            `${name}の代入的呼び出しで計算式が読み取れません。`, word.line, this.filename)}
  ...
  yValueKakko () {
    ...
    if (v === null) {
      const v2 = this.get()
      throw new NakoSyntaxError('(...)の解析エラー。' + this.nodeToStr(v2, { depth: 1 }) + 'の近く', t.line, this.filename)
    }
    if (!this.check(')'))
      {throw new NakoSyntaxError('(...)の解析エラー。' + this.nodeToStr(v, { depth: 1 }) + 'の近く', t.line, this.filename)}
